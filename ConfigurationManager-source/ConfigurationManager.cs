...
namespace ConfigurationManager
{
    ...
    // Upstream declares a hard BepInDependency on _shudnal.ConditionalConfigSync
    // here -- removed along with the rest of that integration (see
    // ConfigSynchronizationInfo.cs), so this plugin doesn't refuse to load
    // just because that mod isn't installed.
    ...
    public partial class ConfigurationManager : BaseUnityPlugin
    {
        ...
        public static ConfigEntry<bool> _compactConfigList;
        ...
        public static ConfigEntry<string> _compactListText;
        public static ConfigEntry<string> _compactListTextTooltip;
        ...
        public static ConfigEntry<Color> _changedSynchronizationPolicyColor;
...
#pragma warning disable IDE1006 // Naming Styles
        // Upstream distinguishes "conditionally server-controlled" from
        // "always server-controlled" via ConditionalConfigSync's extended
        // ConfigSync; not available here (that integration was dropped, see
        // ConfigSynchronizationInfo.cs), so both helpers collapse to plain
        // ServerSync's AddConfigEntry -- any wrapped entry always syncs from
        // the server once connected, same model ValheimQoL/AzuCraftyBoxes
        // already use. `synchronizedSetting` is accepted for call-site
        ...
        {
            return configSync.AddConfigEntry(Config.Bind(group, name, defaultValue, description)).SourceConfig;
        }
